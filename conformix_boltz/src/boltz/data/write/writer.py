...
class BoltzWriter(BasePredictionWriter):
    ...
    def write_on_batch_end(
        ...
        input_dict: dict[str, Tensor] = {},  # noqa: ARG002
        rmsd: list[float] = None,  # noqa: ARG002
    ) -> None:
        ...
        for record, coord, pad_mask in zip(records, coords, pad_masks):
            ...
            for model_idx in range(coord.shape[0]):
                ...
                if "plddt" in prediction:
                    ...
                    # ddr add info about ESS and probs
                    if "ess_trace" in prediction:
                        confidence_summary_dict["ess_trace"] = prediction["ess_trace"].cpu().tolist()

                    # JKARA: append additional information to the confidence summary
                    if input_dict:
                        for key in input_dict:
                            confidence_summary_dict[key] = input_dict[key]
                    if rmsd:
                        confidence_summary_dict["rmsd"] = rmsd[model_idx]
