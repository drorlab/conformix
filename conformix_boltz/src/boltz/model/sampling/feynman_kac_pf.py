...
def smc_FK(M, G, resample_strategy, T, P,  ess_threshold, 
    ...
    if log_xt_trace:
        ...
        xt_trace = None
        grad_log_potential_xt_trace = None
    ...
    for t in time_range:
        ...
        log_wt, logp_y_given_x0, grad_log_potential_xt = G(t=t, xtp1=xtp1, xt=xt, extra_vals=extra_vals) # unnormalized 
        log_w = log_wtp1 + log_wt
        ...
        if log_xt_trace:
            ...
            if xt_trace[-1].shape == grad_log_potential_xt.shape: # true except at the last step
                grad_log_potential_xt_trace.append(xt_trace[-1] + grad_log_potential_xt.cpu()) # show direction
        ess_trace.append(compute_ess_from_log_w(log_w).cpu())
        log_w_trace.append(log_w.cpu())
        logp_y_given_x0_trace.append(logp_y_given_x0.cpu())
    ...
    logp_y_given_x0_trace = torch.stack(logp_y_given_x0_trace, dim=0)
    grad_log_potential_xt_trace = torch.stack(grad_log_potential_xt_trace, dim=0)
    ...
    return xt, log_w, normalized_w, resample_indices_trace, ess_trace, log_w_trace, xt_trace, logp_y_given_x0_trace, grad_log_potential_xt_trace
