...
from boltz.model.sampling import (
    mg_wrapper, feynman_kac_pf
)
...
class ConformixAtomDiffusion(AtomDiffusion):
    """Custom version of AtomDiffusion with modified forward pass."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        Extend the AtomDiffusion class.

        Parameters
        ----------
        
        *args, **kwargs :
            All additional arguments passed to AtomDiffusion
        """
        # Initialize the parent class without reinitializing its state
        # We skip AtomDiffusion's __init__ and directly copy attributes
        # super().__init__(**atom_diffusion_instance.__dict__)
        # Copy all attributes from the provided instance
    ...
    def sample_twisted(
        ...
        multiplicity=1,
        train_accumulate_token_repr=False,
        twist_fn: Callable = None,
        ess_threshold=1/3,
        **network_condition_kwargs,
    ):
        """Sample with twisted diffusion."""
        ...
        # For ConforMix: Never set gammas to zero, keep as gamma_0 for whole time to ensure non-zero variance in TDS code
        # gammas = torch.where(sigmas > self.gamma_min, self.gamma_0, 0.0)
        gammas = torch.ones_like(sigmas) * self.gamma_0
        sigmas_and_gammas = list(zip(sigmas[:-1], sigmas[1:], gammas[1:]))
        ...
        def _denoise(atom_pos: torch.Tensor, t_hat: torch.Tensor, multiplicity: int) -> torch.Tensor:
            ## ConforMix: denoise with gradients enabled
            with torch.enable_grad():
                atom_coords_denoised, token_a = self.preconditioned_network_forward(
                    atom_pos,
                    t_hat, ## same as sigma_hat in Chai
                    training=False,
                    network_condition_kwargs=dict(
                        multiplicity=multiplicity,
                        model_cache=model_cache,
        ...
        sampler = mg_wrapper.TwistedDDPM(
            sigmas, 
            gammas,
            atom_mask,
            noise_scale=self.noise_scale,
            num_timesteps=num_sampling_steps,  # num_steps
            particle_base_shape=(atom_mask.shape[1], 3),
            denoise_fn=_denoise,
            classifier_prob_fn=twist_fn,
            device=self.device,
            num_diffn_samples=multiplicity,
        )

        final_sample, log_w, normalized_w, resample_indices_trace, ess_trace, \
        log_w_trace, xt_trace, logp_y_given_x0_trace, grad_log_potential_xt_trace = \
            feynman_kac_pf.smc_FK(
            M=partial(sampler.M, device=self.device),
            G=sampler.G,
            resample_strategy="systematic", # this is the one they use
            T=200, # final timestep
            P=multiplicity, # num particles
            ess_threshold=ess_threshold, # resample if (effective sample size) < ess_threshold * P
            verbose=True,
            log_xt_trace=True,
            extra_vals=dict(batch_p=25), # largest num of particles per batch (needs to fit in GPU mem)
        )

        atom_coords = final_sample

        return dict(
            sample_atom_coords=atom_coords, 
            diff_token_repr=token_repr,
            logp_y_given_x0_trace=logp_y_given_x0_trace,
            grad_log_potential_xt_trace=grad_log_potential_xt_trace,
            log_w_trace=log_w_trace,
            ess_trace=ess_trace,
            xt_trace=xt_trace
        )
