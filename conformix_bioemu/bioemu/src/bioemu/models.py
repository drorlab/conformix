...
class DiGConditionalScoreModel(torch.nn.Module):
    ...
    def forward(self, x: ChemGraph, t: torch.Tensor) -> ChemGraph:
        ...
        # pos_effective = x.pos.requires_grad_()
        # node_orientations_effective = node_orientations_effective.requires_grad_()
        # time_effective = time_effective.requires_grad_()
        ...
        #import torchviz
        # dot = torchviz.make_dot(pos.mean(), dict(self.model_nn.named_parameters()))
        # dot.format = 'png'
        #dot.render('model_fiz', format='png', cleanup=True)
