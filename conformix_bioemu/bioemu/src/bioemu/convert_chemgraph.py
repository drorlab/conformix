...
def filter_unphysical_traj(
    ...
    return traj.slice(matches_all, copy=True), matches_all
...
def save_pdb_and_xtc(
    ...
    twist_dist_final: torch.Tensor | None = None
) -> None:
    """
    ...
    for i in range(batch_size):
        if torch.any(pos[i].isnan()):
            # sometimes with twisting we get nans
            continue
    ...
    if filter_samples:
        ...
        traj, traj_mask = filter_unphysical_traj(traj)
        logger.info(
        ...
        if twist_dist_final is not None:
            twist_dist_final = twist_dist_final[traj_mask].flatten()


    traj.superpose(reference=traj, frame=0)
    traj.save_xtc(xtc_path)
    if twist_dist_final is not None:
        with open(xtc_path.parent / 'bioemu_aux_data.json', 'w') as f:
            json.dump(dict(twist_dist_final=twist_dist_final.tolist()), f)
