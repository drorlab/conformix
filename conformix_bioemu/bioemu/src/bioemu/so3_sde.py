...
def weighted_rigid_align(
    ...
    if not batched:
        true_coords = true_coords.unsqueeze(0)  # [N, 3] -> [1, N, 3]
        pred_coords = pred_coords.unsqueeze(0)  # [N, 3] -> [1, N, 3]
    
    batch_size, num_points, dim = true_coords.shape
    
    # Handle weights - use uniform weights if not provided
    if weights is None:
        weights = torch.ones(batch_size, num_points, device=true_coords.device)
    else:
...
def batched_expm_SO3(Omega_batch):
    """
    Batched exponential map for a batch of skew-symmetric matrices in so(3).
    Args:
        Omega_batch: Tensor of shape (B, 3, 3) — batch of skew-symmetric matrices.
    Returns:
        R_batch: Tensor of shape (B, 3, 3) — batch of rotation matrices in SO(3).
    """
    device = Omega_batch.device
    B = Omega_batch.shape[0]
    I = torch.eye(3, device=device).expand(B, 3, 3)

    # Extract rotation vector from skew matrix (choose one of two equivalent entries)
    wx = Omega_batch[:, 2, 1]  # or -Omega_batch[:, 1, 2]
    wy = Omega_batch[:, 0, 2]  # or -Omega_batch[:, 2, 0]
    wz = Omega_batch[:, 1, 0]  # or -Omega_batch[:, 0, 1]

    # Calculate angle (clamp for numerical stability)
    theta = torch.sqrt(wx**2 + wy**2 + wz**2).clamp(min=1e-8)

    R_vec = torch.hstack((wx, wy, wz))

    # Compute the terms of the Rodrigues formula
    Omega_sq = torch.bmm(Omega_batch, Omega_batch)
    sin_term = (torch.sin(theta) / theta).view(-1, 1, 1) * Omega_batch
    cos_term = ((1 - torch.cos(theta)) / theta**2).view(-1, 1, 1) * Omega_sq

    # Compute the rotation matrix
    R_batch = I + sin_term + cos_term
    return R_batch, R_vec # rotation in matrix and vector format

def update_rotation_matrix_from_gradient(R, grad_R, eta=1e-2):
    """
    Performs one gradient ascent step on SO(3).
    R: current Bx3x3 rotation matrices (torch tensor).
    grad_R: gradient of loss w.r.t. R (same shape).
    eta: step size.
    Returns: updated rotation matrices.
    """
    assert len(R.shape) == 3 and R.shape[1:] == (3, 3)
    assert len(grad_R.shape) == 3 and grad_R.shape[1:] == (3, 3)

    # want R.T @ grad_R
    sk_input = torch.einsum(
            "b r s, b r t -> b s t",
            R,
            grad_R
            )
    skew_part = 0.5 * (sk_input - sk_input.transpose(1, 2))
    #Omega = R @ skew_part  # tangent vector at R

    # want R @ batched_expm_SO3, exponential map update

    # note this is gradient ascent not descent
    R_batch, R_vec = batched_expm_SO3(eta * skew_part)
    R_new = torch.bmm(R, R_batch)
